...
- **Worker**：9 个流水线 Subagent（context → applicability → signals → synthesize → mapping → gap → evidence）
- **Phase 8（脚本）**：`compute_service_decision.py` 确定性产出 `08-decisions/`，不经过 LLM
- **独立 Agent**：`product-decision`（产品级加权聚合，不嵌入流水线）；`policy-decision`（仅手工调试或非 weighted 策略）
- **能力**：每个 Agent 预加载对应 `.claude/skills/*`
- **契约**：`compliance/schemas/` + `compliance/scripts/validate_artifact.py`

```text
/compliance-run  →  coordinator  →  workers (Phase 1–7, 9)  →  compute_service_decision.py (Phase 8)  →  .claude/runs/{runId}/
...
| 路径 | 说明 |
|------|------|
| `.claude/agents/compliance/` | Claude Code：coordinator + 9 流水线 worker + `policy-decision`（手工）+ `product-decision`（独立） |
| `.opencode/agents/` | OpenCode：同上（9 流水线 worker；Phase 8 由脚本执行） |
...
- [ ] Agent 列表含 `compliance-coordinator`、9 个流水线 worker 及独立 `product-decision`
